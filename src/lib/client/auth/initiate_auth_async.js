 ...
 * @returns {Promise<{access_token: string, user_id: string}>} Resolves with an object containing the access token and user ID.
 */
export function initiateAuthAsync(provider) {
  return new Promise((resolve, reject) => {
    ...
    const handleMessage = (event) => {
      ...
      if (event.data?.access_token && event.data?.user_id) {
        cleanup();
        resolve({
