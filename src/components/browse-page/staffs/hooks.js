...
export function useSearchForm() {
  ...
  useEffect(() => {
    const { query, staffSort } = Object.fromEntries(searchParams.entries());
    ...
    if (
      ...
      staffSort
        .split(',')
        .every((ssUrl) =>
          STAFF_SORT.map((ss) => ss.value).includes(ssUrl.trim().toUpperCase())
        )
    ) {
      formValues['staffSort'] = staffSort
        .split(',')
  ...
  useEffect(() => {
    ...
    newSearchParams.set('subject', 'staffs');
