...
export default function CharactersSearchPageComponent() {
    ...
    <Box w="full" as="form" onSubmit={methods.handleSubmit(onSubmit)}>
      <FormProvider {...methods}>
        ...
        >
          ...
          <FormControl label={'Character Sort'}>
            <Controller
              control={methods.control}
              name={'characterSort'}
              render={({ field }) => (
                <ReactSelectCustom
                  ...
                  value={field.value.map((fv) => ({
                    label: CHARACTER_SORT.find((sort) => sort.value === fv)
                      .label,
                    value: fv,
                  }))}
                  onChange={(options) => {
