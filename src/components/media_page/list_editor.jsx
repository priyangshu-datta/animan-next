...
import { useCreateUpdateMediaListEntry } from '@/lib/client/hooks/react_query/useCreateUpdateMediaListEntry';
import { useGetMediaListEntry } from '@/lib/client/hooks/react_query/useGetMediaListEntry';
import { useToggleMediaFavourite } from '@/lib/client/hooks/react_query/useToggleMediaFavourite';
import { useUserCustomLists } from '@/lib/client/hooks/react_query/useUserCustomLists';
...
export default function ListEditor({ openListEditor, onListEditorClose }) {
  ...
  const toggleFavourite = useToggleMediaFavourite(media.type.toLowerCase());
  ...
  useEffect(() => {
    if (mediaListEntry.isSuccess) {
      ...
      reset({
        status:
          mediaListEntry.data?.data?.Media?.mediaListEntry?.status ??
          'PLANNING',
        progress:
          mediaListEntry.data?.data?.Media?.mediaListEntry?.progress ?? 0,
        favourite: mediaListEntry.data.data.Media.isFavourite ?? false,
        score: mediaListEntry.data?.data?.Media?.mediaListEntry?.score ?? 0,
        'start-end-date': dateArray,
        repeat: mediaListEntry.data?.data?.Media?.mediaListEntry?.repeat ?? 0,
        notes: mediaListEntry.data?.data?.Media?.mediaListEntry?.notes ?? '',
        vlProgress:
          mediaListEntry.data?.data?.Media?.mediaListEntry?.progressVolumes ??
          0,
        customLists:
          Object.keys(customLists)
            .filter(
              (key) =>
                mediaListEntry.data?.data?.Media?.mediaListEntry?.customLists[
                  key
                ]
            )
            .sort() ?? [],
        options: [
          ...(mediaListEntry.data?.data?.Media?.mediaListEntry
            ...
            ? ['hiddenFromStatusLists']
            : []),
          ...(mediaListEntry.data?.data?.Media?.mediaListEntry?.private
            ? ['private']
            : []),
        ].sort(),
      });
    }
  }, [mediaListEntry.isSuccess, reset]);
              ...
              >
                <Controller
                  ...
                  rules={{
                    // max: {
                    //   value: maxEpisodes,
                    //   message: `Episode cannot be above than ${maxEpisodes}`,
                    // },
                    min: {
                      value: 1,
                      message: `${
                ...
                >
                  <Controller
                    ...
                    rules={{
                      ...
                      min: {
                        value: 1,
                        message: 'Volume Progress cannot be 0',
                      },
                    }}
                    render={({ field }) => (
                      <Input
